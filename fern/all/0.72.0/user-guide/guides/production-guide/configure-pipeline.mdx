...
subtitle: Add more resources to your pipeline configuration.
slug: user-guide/guides/production-guide/configure-pipeline
---
...
The next section is about associating a [ZenML Model](../starter-guide/track-ml-models.mdx) with this pipeline.
...
So you can see that the YAML config is fairly easy to use and is an important part of the codebase to control the execution of our pipeline. You can read more about how to configure a pipeline in the [how to section](../../../usage/project-setup/use-configuration-files/what-can-be-configured.mdx), but for now, we can move on to scaling our pipeline.
...
>
 As discussed [before](cloud-orchestration.mdx), we are using the [Kubernetes orchestrator](../../../stack-components/orchestrators/kubernetes.mdx) for Azure users. In order to scale compute for the Kubernetes orchestrator, the YAML file needs to look like this:
...
<Info>
Read more about settings in ZenML [here](../../../usage/project-setup/use-configuration-files/runtime-configuration.mdx) and [here](/usage/resource-data-management/training-with-gpus)
</Info>
...
Bear in mind that not every orchestrator supports `ResourceSettings` directly. To learn more, you can read about [ResourceSettings here](../../../usage/project-setup/use-configuration-files/runtime-configuration.mdx), including the ability to [attach a GPU](/usage/resource-data-management/training-with-gpus#1-specify-a-cuda-enabled-parent-image-in-your-dockersettings).
