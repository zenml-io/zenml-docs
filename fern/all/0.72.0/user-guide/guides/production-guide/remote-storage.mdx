...
subtitle: Transitioning to remote artifact storage.
slug: user-guide/guides/production-guide/remote-storage
---
...
The Amazon Web Services S3 Artifact Store flavor is provided by the [S3 ZenML integration](../../../stack-components/artifact-stores/s3.mdx), you need to install it on your local machine to be able to register an S3 Artifact Store and add it to your stack:
...
```
For more information, read the [dedicated S3 artifact store flavor guide](../../../stack-components/artifact-stores/s3.mdx).
  </Tab>
  <Tab title="CGP">
...
The Google Cloud Storage Artifact Store flavor is provided by the [GCP ZenML integration](../../../stack-components/artifact-stores/gcp.mdx), you need to install it on your local machine to be able to register a GCS Artifact Store and add it to your stack:
...
```
For more information, read the [dedicated GCS artifact store flavor guide](../../../stack-components/artifact-stores/gcp.mdx).
  </Tab>
  <Tab title="Azure">
...
The Microsoft Azure Artifact Store flavor is provided by the [Azure ZenML integration](../../../stack-components/artifact-stores/azure.mdx), you need to install it on your local machine to be able to register an Azure Artifact Store and add it to your stack:
...
For more information, read the [dedicated Azure artifact store flavor guide](../../../stack-components/artifact-stores/azure.mdx).
  </Tab>
<Tab title="Other">
...
It is also relatively simple to create a [custom stack component flavor](../../../stack-components/stack-deployment/implement-a-custom-stack-component.mdx) for your use case.
...
First, let's understand what a service connector does. In simple words, a service connector contains credentials that grant stack components access to cloud infrastructure. These credentials are stored in the form of a [secret](../../../usage/project-setup/use-secrets/interact-with-secrets.mdx), and are available to the ZenML server to use. Using these credentials, the service connector brokers a short-lived token and grants temporary permissions to the stack component to access that infrastructure. This diagram represents this process:
<Frame caption="Service Connectors abstract away complexity and implement security best practices
">
  <img src="../../../_assets/user-guide/production-guide/production-2.png" />
</Frame>
<Tabs>
  <Tab title="AWS">
...
When you run that pipeline, ZenML will automatically store the artifacts in the specified remote storage, ensuring that they are preserved and accessible for future runs and by your team members. You can ask your colleagues to connect to the same [ZenML server](deploying-zenml.mdx), and you will notice that if they run the same pipeline, the pipeline would be partially cached, **even if they have not run the pipeline themselves before**.
...
[ZenML Pro](https://zenml.io/pro) features an [Artifact Control Plane](../starter-guide/manage-artifacts.mdx) to visualize artifact versions:
