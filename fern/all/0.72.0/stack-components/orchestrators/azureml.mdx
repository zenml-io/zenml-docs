...
subtitle: Orchestrating your pipelines to run on AzureML.
slug: stack-components/orchestrators/azureml
---
...
<Info>
    Would you like to skip ahead and deploy a full ZenML cloud stack already, including an AzureML orchestrator? Check out the [in-browser stack deployment wizard](../stack-deployment/deploy-a-cloud-stack.mdx), the [stack registration wizard](../stack-deployment/register-a-cloud-stack.mdx), or [the ZenML Azure Terraform module](../stack-deployment/deploy-a-cloud-stack-with-terraform.mdx) for a shortcut on how to deploy & register this stack component.
</Info>

In order to use an AzureML orchestrator, you need to first deploy [ZenML to the cloud](/getting-started/deploying-zenml). It would be recommended to deploy ZenML in the same region as you plan on using for AzureML, but it is not necessary to do so. You must ensure that you are [connected to the remote ZenML server](../../usage/resource-data-management/connecting-to-zenml/connect-in-with-your-user-interactive.mdx) before using this stack component.
...
<Frame caption="AzureML pipeline example">
  <img src="../../_assets/stack-components/stack-components-7.png"/>
</Frame>
