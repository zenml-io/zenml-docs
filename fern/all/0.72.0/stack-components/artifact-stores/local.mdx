...
subtitle: Storing artifacts on your local filesystem.
slug: stack-components/artifact-stores/local
---
...
* only [Orchestrators](/stack-components/orchestrators) running on the local machine, such as the [local Orchestrator](../orchestrators/local.mdx), a [local Kubeflow Orchestrator](../orchestrators/kubeflow.mdx), or a [local Kubernetes Orchestrator](../orchestrators/kubernetes.mdx) can be combined with a local Artifact Store
* only [Model Deployers](/stack-components/model-deployers) that are running locally, such as the [MLflow Model Deployer](../model-deployers/mlflow.mdx), can be used in combination with a local Artifact Store
