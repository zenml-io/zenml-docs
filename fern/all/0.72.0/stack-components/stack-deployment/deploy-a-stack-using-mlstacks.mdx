...
subtitle: Individually deploying different stack components.
slug: stack-components/stack-deployment/deploy-a-stack-using-mlstacks
---


The first step in running your pipelines on remote infrastructure is to deploy all the components that you would need, like an [MLflow tracking server](../experiment-trackers/mlflow.mdx), [Kubeflow orchestrator](../orchestrators/kubeflow.mdx), and more to your cloud.
...
Deploying an end-to-end stack through the ZenML CLI is only possible with the [deployment wizard which does not use mlstacks](deploy-a-cloud-stack.mdx). However, you can use `mlstacks` directly to deploy various types of stacks and [import them into ZenML](https://mlstacks.zenml.ioversions/0.66.0/reference/zenml).
