...
subtitle: Creating a full picture of a ML model using the Model Control Plane
slug: user-guide/guides/starter-guide/track-ml-models
---
...
As discussed in the [Core Concepts](../../../getting-started/core-concepts.mdx), ZenML also contains the notion of a `Model`, which consists of many model versions (the iterations of the model). These concepts are exposed in the `Model Control Plane` (MCP for short).
...
The easiest way to use a ZenML model is to pass a `Model` object as part of a pipeline run. This can be done easily at a pipeline or a step level, or via a [YAML config](../production-guide/configure-pipeline.mdx).
...
When configured at the pipeline or step level, the model will be available through the [StepContext](../../../usage/resource-data-management/track-metrics-metadata/fetch-metadata-within-pipeline.mdx) or [PipelineContext](../../../usage/resource-data-management/track-metrics-metadata/fetch-metadata-within-pipeline.mdx).
