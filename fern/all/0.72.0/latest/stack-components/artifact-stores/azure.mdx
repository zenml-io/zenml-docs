...
subtitle: Storing artifacts using Azure Blob Storage
slug: stack-components/artifact-stores/azure
---
...
To enable these use cases, it is recommended to use [an Azure Service Connector](../../usage/resource-data-management/auth-management/azure-service-connector.mdx) to link your Azure Artifact Store to the remote Azure Blob storage container.
</Note>
    </Tab>
    <Tab title="Azure Service Connector (recommended)">
To set up the Azure Artifact Store to authenticate to Azure and access an Azure Blob storage container, it is recommended to leverage the many features provided by [the Azure Service Connector](../../usage/resource-data-management/auth-management/azure-service-connector.mdx) such as auto-configuration, best security practices regarding long-lived credentials and reusing the same credentials across multiple stack components.
...
When you register the Azure Artifact Store, you can create a [ZenML Secret](../../usage/project-setup/use-secrets/secret-management.mdx) to store a variety of Azure credentials and then reference it in the Artifact Store configuration:
