---
title: Overview
subtitle: Overview of categories of MLOps components and third-party integrations.
slug: stack-components/component-guide
---


If you are new to the world of MLOps, it is often daunting to be immediately faced with a sea of tools that seemingly all promise and do the same things. It is useful in this case to try to categorize tools in various groups in order to understand their value in your toolchain in a more precise manner.

ZenML tackles this problem by introducing the concept of [Stacks and Stack Components](../user-guide/guides/production-guide/understand-stacks.mdx). These stack components represent categories, each of which has a particular function in your MLOps pipeline. ZenML realizes these stack components as base abstractions that standardize the entire workflow for your team. In order to then realize the benefit, one can write a concrete implementation of the [abstraction](stack-deployment/implement-a-custom-stack-component.mdx), or use one of the many built-in [integrations](component-guide.mdx) that implement these abstractions for you.
...
For example, you can orchestrate your ML pipeline workflows using [Airflow](orchestrators/airflow.mdx) or [Kubeflow](orchestrators/kubeflow.mdx), track experiments using [MLflow Tracking](experiment-trackers/mlflow.mdx) or [Weights & Biases](experiment-trackers/wandb.mdx), and transition seamlessly from a local [MLflow deployment](model-deployers/mlflow.mdx) to a deployed model on Kubernetes using [Seldon Core](model-deployers/seldon.mdx).
...
<Frame caption="ZenML is the glue">
  <img src="../_assets/stack-components/stack-components-1.png"/>
</Frame>
...
Before you can use integrations, you first need to install them using `zenml integration install`, e.g., you can install [Kubeflow](orchestrators/kubeflow.mdx), [MLflow Tracking](experiment-trackers/mlflow.mdx), and [Seldon Core](model-deployers/seldon.mdx), using:
