...
subtitle: Building container images with Google Cloud Build
slug: stack-components/image-builders/gcp
---
...
* your stack is mainly composed of other Google Cloud components such as the [GCS Artifact Store](../artifact-stores/gcp.mdx) or the [Vertex Orchestrator](../orchestrators/vertex.mdx).

### How to deploy it

<Info>
    Would you like to skip ahead and deploy a full ZenML cloud stack already, including the Google Cloud image builder? Check out the [in-browser stack deployment wizard](../stack-deployment/deploy-a-cloud-stack.mdx), the [stack registration wizard](../stack-deployment/register-a-cloud-stack.mdx), or [the ZenML GCP Terraform module](../stack-deployment/deploy-a-cloud-stack-with-terraform.mdx) for a shortcut on how to deploy & register this stack component.
</Info>
...
```
* A [GCP Artifact Store](../artifact-stores/gcp.mdx) where the build context will be uploaded, so Google Cloud Build can access it.
* A [GCP container registry](../container-registries/gcp.mdx) where the built image will be pushed.
