...
subtitle: >-
  By default, ZenML uses a logging handler to capture the logs that occur during
  the execution of a step.
slug: usage/development-monitoring/control-logging/view-logs-on-the-dasbhoard
---
...
* In case of a deployed ZenML server, logs for runs on a [local artifact store](../../../stack-components/artifact-stores/local.mdx) will not be accessible. Logs for runs using a [remote artifact store](../../../user-guide/guides/production-guide/remote-storage.mdx) **may be** accessible, if the artifact store has been configured with a [service connector](../../resource-data-management/auth-management/service-connectors-guide.mdx). Please read [this chapter](../../../user-guide/guides/production-guide/remote-storage.mdx) of the production guide to learn how to configure a remote artifact store with a service connector.
...
<Frame caption="Displaying step logs on the dashboard">
  <img src="../../../_assets/how-to/stack-46.png" />
</Frame>

<Note>
    If you do not want to store the logs for your pipeline (for example due to performance reduction or storage limits), you can follow [these instructions](enable-or-disable-logs-storing.mdx).
</Note>
