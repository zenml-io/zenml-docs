...
subtitle: >-
  Here is an example of a sample YAML file, with the most important
  configuration highlighted.
slug: usage/project-setup/use-configuration-files/what-can-be-configured
---


For brevity, we have removed all possible keys. To view a sample file with all possible keys, refer to [this page](autogenerate-a-template-yaml-file.mdx).
...
* `enable_cache`: Utilize [caching](../../pipelines/build-pipelines/control-caching-behavior.mdx) or not.
* `enable_step_logs`: Enable tracking [step logs](../../development-monitoring/control-logging/enable-or-disable-logs-storing.mdx).
...
Specifies the ZenML [Model](../../../user-guide/guides/starter-guide/track-ml-models.mdx) to use for this pipeline.
...
A dictionary of JSON-serializable [parameters](../../pipelines/build-pipelines/use-pipeline-step-parameters.mdx) specified at the pipeline or step level. For example:
...
Important note, in the above case, the value of the step would be the one defined in the `steps` key (i.e. 0.001). So the YAML config always takes precedence over pipeline parameters that are passed down to steps in code. Read [this section for more details](configuration-hierarchy.mdx).
...
Settings are special runtime configurations of a pipeline or a step that require a [dedicated section](runtime-configuration.mdx). In short, they define a bunch of execution configuration such as Docker building and resource settings.
...
The `source` of the [failure and success hooks](../../pipelines/build-pipelines/use-failure-success-hooks.mdx) can be specified.
...
* `outputs`: This is configuration of the output artifacts of this step. This is further keyed by output name (by default, step outputs [are named output](../../resource-data-management/handle-data-artifacts/return-multiple-outputs-from-a-step.mdx)). The most interesting configuration here is the `materializer_source`, which is the UDF path of the materializer in code to use for this output (e.g. `materializers.some_data.materializer.materializer_class`). Read more about this source path [here](../../resource-data-management/handle-data-artifacts/handle-custom-data-types.mdx).
