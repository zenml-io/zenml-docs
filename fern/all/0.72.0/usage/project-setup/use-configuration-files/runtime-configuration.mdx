...
subtitle: Using settings to configure runtime configuration.
slug: usage/project-setup/use-configuration-files/runtime-configuration
---
...
A good example of this is the [MLflow Experiment Tracker](../../../stack-components/experiment-trackers/mlflow.mdx), where configuration which remains static such as the `tracking_url` is sent through at registration time, while runtime configuration such as the `experiment_name` (which might change every pipeline run) is sent through as runtime settings.
...
For example, the [SagemakerStepOperator](../../../stack-components/step-operators/sagemaker.mdx) supports passing in [estimator\_args](https://sdkdocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-aws/#zenml.integrations.aws.flavors.sagemaker%5Fstep%5Foperator%5Fflavor.SagemakerStepOperatorSettings). The way to specify this would be to use the key `step_operator`
