...
subtitle: Inspecting a finished pipeline run and its outputs.
slug: usage/pipelines/build-pipelines/fetching-pipelines
---
...
Check out the [ZenML Client Documentation](../../../sdk/python-client.mdx) for more information on the `Client` class and its purpose.
...
The `pipeline_configuration` is an object that contains all configurations of the pipeline and pipeline run, including the [pipeline-level settings](../../../user-guide/guides/production-guide/configure-pipeline.mdx), which we will learn more about later:
...
<Info>
    If you're only calling each step once inside your pipeline, the **invocation ID** will be the same as the name of your step. For more complex pipelines, check out [this page](using-a-custom-step-invocation-id.mdx) to learn more about the invocation ID.
</Info>
...
<Frame caption="output.visualize() Output">
    <img src="../../../_assets/how-to/setting-up-a-project-repository/pipelines-1.png"/>
</Frame>
<Info>
...
<Info>
    As shown in the example, we can get additional information about the current run using the `StepContext`, which is explained in more detail in the [advanced docs](../../resource-data-management/track-metrics-metadata/fetch-metadata-within-steps.mdx).
</Info>
