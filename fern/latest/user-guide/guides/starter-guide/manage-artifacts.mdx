...
subtitle: Understand and adjust how ZenML versions your data.
slug: user-guide/guides/starter-guide/manage-artifacts
---
...
<Note>
Since custom versions cannot be duplicated, the above step can only be run once successfully. To avoid altering your code frequently, consider using a [YAML config](../production-guide/configure-pipeline.mdx) for artifact versioning.
</Note>
...
<Info>
Calls of `Client` methods like `get_artifact_version` directly inside the pipeline code makes use of ZenML's [late materialization](../../../usage/resource-data-management/handle-data-artifacts/load-artifacts-into-memory.mdx) behind the scenes.
</Info>
