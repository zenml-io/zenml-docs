...
subtitle: Orchestrate using cloud resources.
slug: user-guide/guides/production-guide/cloud-orchestration
---
...
These, along with [remote storage](remote-storage.mdx), complete a basic cloud stack where our pipeline is entirely running on the cloud.
...
While there are detailed docs on [how to set up a Skypilot orchestrator](../../../stack-components/orchestrators/skypilot-vm.mdx) and a [container registry](/stack-components/container-registries) on each public cloud, we have put the most relevant details here for convenience:
<Tabs>
  <Tab title="AWS">
...
Once the service connector is set up, we can register [a Skypilot orchestrator](../../../stack-components/orchestrators/skypilot-vm.mdx):
...
The next step is to register [an AWS container registry](../../../stack-components/container-registries/aws.mdx). Similar to the orchestrator, we will use our connector as we are setting up the container registry:
...
For more information, you can always check the [dedicated Skypilot orchestrator guide](../../../stack-components/orchestrators/skypilot-vm.mdx).
  </Tab>
  <Tab title="GCP">
...
The next step is to register [a GCP container registry](../../../stack-components/container-registries/gcp.mdx). Similar to the orchestrator, we will use our connector as we are setting up the container registry:
...
<Tab title="Azure">
As of [v0.60.0](https://github.com/zenml-io/zenml/releases/tag/0.60.0), alongside the switch to `pydantic` v2, due to an incompatibility between the new version `pydantic` and the `azurecli`, the `skypilot[azure]` flavor can not be installed at the same time. Therefore, for Azure users, an alternative is to use the [Kubernetes Orchestrator](../../../stack-components/orchestrators/kubernetes.mdx). You can easily deploy a Kubernetes cluster in your subscription using the [Azure Kubernetes Service](https://azure.microsoft.com/en-us/products/kubernetes-service).
...
Once the service connector is set up, we can register [a Kubernetes orchestrator](../../../stack-components/orchestrators/kubernetes.mdx):
...
The next step is to register [an Azure container registry](../../../stack-components/container-registries/azure.mdx). Similar to the orchestrator, we will use our connector as we are setting up the container registry.
...
For more information, you can always check the [dedicated Kubernetes orchestrator guide](../../../stack-components/orchestrators/kubernetes.mdx).
  </Tab>
</Tabs>
...
Curious to see what other stacks you can create? The [Component Guide](../../../stack-components/component-guide.mdx) has an exhaustive list of various artifact stores, container registries, and orchestrators that are integrated with ZenML. Try playing around with more stack components to see how easy it is to switch between MLOps stacks with ZenML.
