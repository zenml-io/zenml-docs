...
subtitle: Executing individual steps in SageMaker.
slug: stack-components/step-operators/sagemaker
---
...
* you have access to SageMaker. If you're using a different cloud provider, take a look at the [Vertex](vertex.mdx) or [AzureML](azureml.mdx) step operators.
...
* An [AWS container registry](../container-registries/aws.mdx) as part of our stack. Take a look [here](/stack-components/container-registries/aws#how-to-deploy-it) for a guide on how to set that up.
...
* If using a [local orchestrator](../orchestrators/local.mdx): ZenML will try to implicitly authenticate to AWS via the `default` profile in your local [AWS configuration file](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html). Make sure this profile has permissions to create and manage SageMaker runs (e.g. [the AmazonSageMakerFullAccess managed policy](https://docs.aws.amazon.com/sagemaker/latest/dg/security-iam-awsmanpol.html) permissions).
...
For additional configuration of the SageMaker step operator, you can pass `SagemakerStepOperatorSettings` when defining or running your pipeline. Check out the [SDK docs](https://sdkdocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-aws/#zenml.integrations.aws.flavors.sagemaker%5Fstep%5Foperator%5Fflavor.SagemakerStepOperatorSettings) for a full list of available attributes and [this docs page](../../usage/project-setup/use-configuration-files/runtime-configuration.mdx) for more information on how to specify settings.
