...
subtitle: Storing artifacts in an AWS S3 bucket.
slug: stack-components/artifact-stores/s3
---
...
Running ZenML pipelines with [the local Artifact Store](local.mdx) is usually sufficient if you just want to evaluate ZenML or get started quickly without incurring the trouble and the cost of employing cloud storage services in your stack. However, the local Artifact Store becomes insufficient or unsuitable if you have more elaborate needs for your project:
...
To enable these use-cases, it is recommended to use [an AWS Service Connector](../../usage/resource-data-management/auth-management/aws-service-connector.mdx) to link your S3 Artifact Store to the remote S3 bucket.
    </Tab>
    <Tab title="AWS Service Connector (recommended)">
    To set up the S3 Artifact Store to authenticate to AWS and access an S3 bucket, it is recommended to leverage the many features provided by [the AWS Service Connector](../../usage/resource-data-management/auth-management/aws-service-connector.mdx) such as auto-configuration, best security practices regarding long-lived credentials and fine-grained access control and reusing the same credentials across multiple stack components.
...
When you register the S3 Artifact Store, you can [generate an AWS access key](https://aws.amazon.com/premiumsupport/knowledge-center/create-access-key/), store it in a [ZenML Secret](../../usage/project-setup/use-secrets/secret-management.mdx) and then reference it in the Artifact Store configuration.
