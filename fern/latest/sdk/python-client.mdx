---
title: Python Client
subtitle: Interacting with your ZenML instance through the ZenML Client.
slug: sdk/python-client
---
...
* **Schedules**: Metadata about the schedules that you have used to [schedule pipeline runs](../usage/pipelines/build-pipelines/schedule-a-pipeline.mdx).
...
* **Code Repositories**: The git code repositories that you have connected with your ZenML instance. See [here](../user-guide/guides/production-guide/connect-code-repository.mdx) for more information.

<Info>
Checkout the [documentation on fetching runs](../usage/pipelines/build-pipelines/fetching-pipelines.mdx) for more information on the various ways how you can fetch and use the pipeline, pipeline run, step run, and artifact resources in code.
</Info>
...
* **Flavors**: The [stack component flavors](/getting-started/core-concepts#flavor) available to you, including:
   * Built-in flavors like the [local orchestrator](../stack-components/orchestrators/local.mdx),
   * Integration-enabled flavors like the [Kubeflow orchestrator](../stack-components/orchestrators/kubeflow.mdx),
   * Custom flavors that you have [created yourself](../stack-components/stack-deployment/implement-a-custom-stack-component.mdx).
* **User**: The users registered in your ZenML instance. If you are running locally, there will only be a single `default` user.
* **Secrets**: The infrastructure authentication secrets that you have registered in the [ZenML Secret Store](../usage/project-setup/use-secrets/interact-with-secrets.mdx).
