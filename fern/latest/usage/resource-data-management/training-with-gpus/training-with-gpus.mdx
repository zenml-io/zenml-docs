---
title: Overview
subtitle: Ensuring your pipelines or steps run on GPU-backed hardware.
slug: usage/resource-data-management/training-with-gpus/training-with-gpus
---
...
If the underlying [orchestrator](/stack-components/orchestrators) in your stack then supports specifying resources, this setting will attempt to secure these resources. Some orchestrators (like the [Skypilot orchestrator](../../../stack-components/orchestrators/skypilot-vm.mdx)) do not support `ResourceSettings` directly, but rather use their `Orchestrator` specific settings to achieve the same effect:
