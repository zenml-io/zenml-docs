...
<Info>
    Here we are calling one pipeline from within another pipeline, so functionally the `data_loading_pipeline` is functioning as a step within the `training_pipeline`, i.e. the steps of the former are added to the latter. Only the parent pipeline will be visible in the dashboard. In order to actually trigger a pipeline from another, see [here](latest/usage/pipelines/trigger-pipelines/trigger-a-pipeline-from-another)
</Info>


<CardGroup cols={2}>
<Card title="Learn about orchestrators" icon="server" href="latest/stack-components/orchestrators" horizontal />
</CardGroup>
