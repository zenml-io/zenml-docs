...
[HyperAI](https://www.hyperai.ai) is a cutting-edge cloud compute platform designed to make AI accessible for everyone. The HyperAI orchestrator is an [orchestrator](latest/stack-components/orchestrators) flavor that allows you to easily deploy your pipelines on HyperAI instances.
...
[Scheduled pipelines](latest/usage/pipelines/build-pipelines/schedule-a-pipeline) are supported by the HyperAI orchestrator. Currently, the HyperAI orchestrator supports the following inputs to `Schedule`:
