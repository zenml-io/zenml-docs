...
<Info>
    Would you like to skip ahead and deploy a full ZenML cloud stack already, including an AzureML orchestrator? Check out the [in-browser stack deployment wizard](latest/stack-components/stack-deployment/deploy-a-cloud-stack), the [stack registration wizard](latest/stack-components/stack-deployment/register-a-cloud-stack), or [the ZenML Azure Terraform module](latest/stack-components/stack-deployment/deploy-a-cloud-stack-with-terraform) for a shortcut on how to deploy & register this stack component.
</Info>

In order to use an AzureML orchestrator, you need to first deploy [ZenML to the cloud](latest/getting-started/deploying-zenml). It would be recommended to deploy ZenML in the same region as you plan on using for AzureML, but it is not necessary to do so. You must ensure that you are [connected to the remote ZenML server](latest/usage/resource-data-management/connecting-to-zenml/connect-in-with-your-user-interactive) before using this stack component.
...
For each pipeline run, ZenML will build a Docker image called `<CONTAINER_REGISTRY_URI>/zenml:<PIPELINE_NAME>` which includes your code and use it to run your pipeline steps in AzureML. Check out [this page](latest/usage/project-setup/customize-docker-builds) if you want to learn more about how ZenML builds these images and how you can customize them.
