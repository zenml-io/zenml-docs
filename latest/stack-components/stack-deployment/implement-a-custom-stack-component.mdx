...
As each component defines a different interface, make sure to check out the base class definition of the component type that you want to implement and also check out the [documentation on how to extend specific stack components](latest/stack-components/stack-deployment/implement-a-custom-stack-component#extending-specific-stack-components).

<Info>
    If you would like to automatically track some metadata about your custom stack component with each pipeline run, you can do so by defining some additional methods in your stack component implementation class as shown in the [Tracking Custom Stack Component Metadata](latest/usage/resource-data-management/track-metrics-metadata/fetch-metadata-within-steps) section.
</Info>
...
You can read more about the differences [here](latest/usage/project-setup/use-configuration-files/runtime-configuration).
...
<Info>
    You can pass sensitive configuration values as [secrets](latest/usage/project-setup/use-secrets/interact-with-secrets) by defining them as type `SecretField` in the configuration class.
</Info>
...
| ------------------------------------------------------------------- | ----------------------------------------------------------------- |
| [Orchestrator](latest/stack-components/orchestrators/custom)              | Orchestrating the runs of your pipeline                           |
| [Artifact Store](latest/stack-components/artifact-stores/custom)          | Storage for the artifacts created by your pipelines               |
| [Container Registry](latest/stack-components/container-registries/custom) | Store for your containers                                         |
| [Step Operator](latest/stack-components/step-operators/custom)            | Execution of individual steps in specialized runtime environments |
| [Model Deployer](latest/stack-components/model-deployers/custom)          | Services/platforms responsible for online model serving           |
| [Feature Store](latest/stack-components/feature-stores/custom)            | Management of your data/features                                  |
| [Experiment Tracker](latest/stack-components/experiment-trackers/custom)  | Tracking your ML experiments                                      |
| [Alerter](latest/stack-components/alerters/custom)                        | Sending alerts through specified channels                         |
| [Annotator](latest/stack-components/annotators/custom)                    | Annotating and labeling data                                      |
| [Data Validator](latest/stack-components/data-validators/custom)          | Validating and monitoring your data                               |
