...
2. Create a class that inherits from [the BaseArtifactStoreConfig class](/latest/stack-components/artifact-stores/custom) and fill in the `SUPPORTED_SCHEMES` based on your file system.
3. Bring both of these classes together by inheriting from [the BaseArtifactStoreFlavor class](/latest/stack-components/artifact-stores/custom).
