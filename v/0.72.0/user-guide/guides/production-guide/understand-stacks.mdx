...
Now that we have ZenML deployed, we can take the next steps in making sure that our machine learning workflows are production-ready. As you were running [your first pipelines](/latest/versions/0.66.0/user-guide/starter-guide/create-an-ml-pipeline), you might have already noticed the term `stack` in the logs and on the dashboard.
...
There are many more components that you can add to your stacks, like experiment trackers, model deployers, and more. You can see all supported stack component types in a single table view [here](/latest/versions/0.66.0/stack-components/component-guide)

Perhaps the most important stack component after the orchestrator and the artifact store is the [container registry](/latest/versions/0.66.0/stack-components/container-registries). A container registry stores all your containerized images, which hold all your code and the environment needed to execute them. We will learn more about them in the next section!
...
Let's use the pipeline in our starter project from the [previous guide](/latest/versions/0.66.0/user-guide/starter-guide/starter-project) to see it in action.
