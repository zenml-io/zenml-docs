...
The easiest and fastest way to get running on the cloud is by using the `deploy` CLI command. It currently only supports deploying to Kubernetes on managed cloud services. You can check the [overview page](/v/0.67.0/versions/0.66.0/getting-started/deploying-zenml#deploying-a-zenml-server) to learn about other options that you have.

Before we begin, it will help to understand the [architecture](/v/0.67.0/versions/0.66.0/getting-started/deploying-zenml) around the ZenML server and the database that it uses. Now, depending on your setup, you may find one of the following scenarios relevant.
