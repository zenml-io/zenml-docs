...
* The [resources](/v/0.66.0/usage/resource-data-management/training-with-gpus#specify-resource-requirements-for-steps) required for a step
* Configuring the [containerization](/v/0.66.0/usage/project-setup/customize-docker-builds) process of a pipeline (e.g. What requirements get installed in the Docker image)
...
* **General settings** that can be used on all ZenML pipelines. Examples of these are:
   * [DockerSettings](/v/0.66.0/usage/project-setup/customize-docker-builds) to specify Docker settings.
   * [ResourceSettings](/v/0.66.0/usage/resource-data-management/training-with-gpus) to specify resource settings.
...
For example, the [SagemakerStepOperator](/v/0.66.0/stack-components/step-operators/sagemaker) supports passing in [estimator\_args](https://sdkdocs.zenml.io/latest/integration%5Fcode%5Fdocs/integrations-aws/#zenml.integrations.aws.flavors.sagemaker%5Fstep%5Foperator%5Fflavor.SagemakerStepOperatorSettings). The way to specify this would be to use the key `step_operator`
