...
A step can also be specified as a local user-defined function path (of the form `mymodule.myfile.my_function`). This is particularly useful when defining the hooks via a [YAML Config](/v/0.66.0/usage/project-setup/use-configuration-files).
...
>
_To set up the local environment used below, follow the recommendations from the_ [_Project templates_](/v/0.66.0/usage/project-setup/setting-up-a-project-repository/using-project-templates)_._

In [steps/alerts/notify\_on.py](https://github.com/zenml-io/zenml/blob/release/0.66.0/examples/e2e/steps/alerts/notify%5Fon.py), you will find a step to notify the user about success and a function used to notify the user about step failure using the [Alerter](/v/0.66.0/stack-components/alerters) from the active stack.
...
Similar as for regular ZenML steps, you can use the [StepContext](/v/0.66.0/usage/resource-data-management/track-metrics-metadata/fetch-metadata-within-steps) to access information about the current pipeline run or step inside your hook function:
...
Inside the helper function `build_message()`, you will find an example on how developers can work with [StepContext](/v/0.66.0/usage/resource-data-management/track-metrics-metadata/fetch-metadata-within-steps) to form a proper notification:
...
A common use case is to use the [Alerter](/v/0.66.0/stack-components/alerters) component inside the failure or success hooks to notify relevant people. It is quite easy to do this:
