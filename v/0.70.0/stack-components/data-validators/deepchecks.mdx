...
You should consider one of the other [Data Validator flavors](/v/0.70.0/versions/0.66.0/stack-components/data-validators#data-validator-flavors) if you need a different set of data validation features.
...
* instantiate, configure and insert one or more of [the standard Deepchecks steps](/v/0.70.0/versions/0.66.0/stack-components/data-validators/deepchecks#the-deepchecks-standard-steps) shipped with ZenML into your pipelines. This is the easiest way and the recommended approach, but can only be customized through the supported step configuration parameters.
* call the data validation methods provided by [the Deepchecks Data Validator](/v/0.70.0/versions/0.66.0/stack-components/data-validators/deepchecks#the-deepchecks-data-validator) in your custom step implementation. This method allows for more flexibility concerning what can happen in the pipeline step, but you are still limited to the functionality implemented in the Data Validator.
* [use the Deepchecks library directly](/v/0.70.0/versions/0.66.0/stack-components/data-validators/deepchecks#call-deepchecks-directly) in your custom step implementation. This gives you complete freedom in how you are using Deepchecks' features.
...
The current ZenML version has a limitation in its base Docker image that requires a workaround for _all_ pipelines using Deepchecks with a remote orchestrator (e.g. [Kubeflow](/v/0.70.0/versions/0.66.0/stack-components/orchestrators/kubeflow) , [Vertex](/v/0.70.0/versions/0.66.0/stack-components/orchestrators/vertex)). The limitation being that the base Docker image needs to be extended to include binaries that are required by `opencv2`, which is a package that Deepchecks requires.
...
Then, place the following snippet above your pipeline definition. Note that the path of the `dockerfile` are relative to where the pipeline definition file is. Read [the containerization guide](/v/0.70.0/versions/0.66.0/how-to/customize-docker-builds) for more details:
...
The integration doesn't yet include standard steps for computer vision, but you can still write your own custom steps that call [the Deepchecks Data Validator API](/v/0.70.0/versions/0.66.0/stack-components/data-validators/deepchecks#the-deepchecks-data-validator) or even [call the Deepchecks library directly](/v/0.70.0/versions/0.66.0/stack-components/data-validators/deepchecks#call-deepchecks-directly).
...
The arguments that the Deepchecks Data Validator methods can take in are the same as those used for [the Deepchecks standard steps](/v/0.70.0/versions/0.66.0/stack-components/data-validators/deepchecks#the-deepchecks-standard-steps).
...
Alternatively, if you are running inside a Jupyter notebook, you can load and render the suites and results using the [artifact.visualize() method](/v/0.70.0/versions/0.66.0/how-to/visualize-artifacts), e.g.:
