...
* In case of a deployed ZenML server, logs for runs on a [local artifact store](/v/0.70.0/versions/0.66.0/stack-components/artifact-stores/local) will not be accessible. Logs for runs using a [remote artifact store](/v/0.70.0/versions/0.66.0/user-guide/production-guide/remote-storage) **may be** accessible, if the artifact store has been configured with a [service connector](/v/0.70.0/versions/0.66.0/how-to/auth-management/service-connectors-guide). Please read [this chapter](/v/0.70.0/versions/0.66.0/user-guide/production-guide/remote-storage) of the production guide to learn how to configure a remote artifact store with a service connector.
...
<Note>
    If you do not want to store the logs for your pipeline (for example due to performance reduction or storage limits), you can follow [these instructions](/v/0.70.0/versions/0.66.0/how-to/control-logging/enable-or-disable-logs-storing).
</Note>
