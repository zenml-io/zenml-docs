...
Usually, ZenML [stores step logs in the artifact store](/v/0.70.0/versions/0.66.0/how-to/control-logging/enable-or-disable-logs-storing), but this can sometimes cause performance bottlenecks, especially if the code utilizes progress bars.
...
Please see [our full page](/v/0.70.0/versions/0.66.0/reference/global-settings#usage-analytics) on what analytics are tracked and how you can opt out, but the quick summary is that you can set this to `false` if you want to opt out of analytics.
...
For more information on server configuration, see the [ZenML Server documentation](/v/0.70.0/versions/0.66.0/getting-started/deploying-zenml/deploy-with-docker#zenml-server-configuration-options) for more, especially the section entitled "ZenML server configuration options".
